...
def patched_get_module_paths(module):
    """Skip torch.classes module inspection safely."""
    try:
        if hasattr(module, "__name__") and "torch._classes" in module.__name__:
            return []
...
# Removed problematic PDFKit configuration
# No longer using pdfkit to avoid OSError
...
def delete_history_entry(username, timestamp):
    ...
    if username in users:
        users[username]["history"] = [entry for entry in users[username].get("history", []) if
    ...
    if st.session_state.logged_in_user == "guest":
        if st.sidebar.button("Return to Main Page"):
            ...
            st.session_state.update({
                ...
                "selected_history_id": None,
                "auth_page": "main",
                "query_params": {}  # Clear any query parameters
            })
            st.rerun()
    else:
        if st.sidebar.button("Logout"):
            # Comprehensive session state reset
            keys_to_preserve = ['_pages', '_script_run_count']  # Preserve Streamlit internal states
        ...
        def create_pdf():
            try:
                ...
                user_info = Paragraph(
                    ...
                    f"<b>Generated on:</b> {datetime.now().strftime('%Y-%m-%d')}",
                    styles["Normal"]
                )
